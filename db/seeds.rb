...
p = Post.create(title: 'Levitating Sheep - Multiple Backgrounds for Simple Parallax Effect',
            body:
            %{One of my recent discoveries while styling out my site is that CSS3 lets us specify multiple background images and they get stacked on top of each other, kinda like photoshop layers. I wrote a quick script that pans my background images in order to create a simple parallax effect. To test this, I thought it would be cool to take a simple sheep, and make her levitate (or fall depending on which way you're scrolling...). <img src="/assets/posts/parallax/sheep.jpg" style="display:block;margin:10px auto;box-shadow: 2px 2px 0 #A1A1A1;" /> The basic premise is, as a user is scrolling through a post, a background image will also scroll from top to bottom within the post's background. I thought this looked pretty cool as a first run with only one background image, but then I had the idea to add an additional background image layer so that I can use 2 layers of scrolling backgrounds. If the top layer's image has a greater height, then it will scroll faster than the bottom layer's image, which esentially creates a simple parallax effect. That's the effect that I used to get these levitating sheep, with some being farther away and some being closer. Heres the simplified css: <code>background-image: url(big-falling-sheep.png), url(small-falling-sheep.png);<br />background-position: 50% 0%;</code> To get it to work, I just simply adjust the background-position from 50% 0% to 50% 100% using javascript as the user scrolls along the page.},
